...
protected:
	...
	void call_leave__( const stateData_t& state , const  CONDITION_DATA_TYPE& data ){
		auto tmp = state , tmp_d = data;
        pt_producer__->sendTo( pt_consumer__ , std::move( tmp ) , private__::emEvent::EVT_LEAVE , std::move( tmp_d ) );
	}
	// 通知进入状态的操作
	void call_ent__( const stateData_t& state , const CONDITION_DATA_TYPE& data ){
		auto tmp = state, tmp_d = data;
        pt_producer__->sendTo( pt_consumer__ , std::move( tmp ) , private__::emEvent::EVT_ENT , std::move( tmp_d ) );
	}
public:
	// 这个是构造函数，其中初始化了中介者模式
	...
	void start( bool sw ){
        ...
        if( sw ){
			...
			auto tmp = m_start__;
			// 发送就绪通知
            pt_producer__->sendTo( pt_consumer__ , std::move( tmp ),  private__::emEvent::EVT_READY ,
			...
			for( auto item : it->second ){
				if( item.m_condition( data ) ){
                    ...
                    if( item.m_to == m_end__ ){
						auto tmp = m_end__;
                        pt_producer__->sendTo( pt_consumer__ , std::move( tmp ),  private__::emEvent::EVT_END ,
                                             std::is_arithmetic<CONDITION_DATA_TYPE>::value?CONDITION_DATA_TYPE():(CONDITION_DATA_TYPE)0.0 );
                    }
