...
namespace fsm2{
	...
	static const bool TRIGER_RUN = false;
        ...
        template< typename dataType ,
		  typename realT = typename std::decay< dataType >::type
		  /*
		  typename realT = typename std::conditional<
			  std::is_pointer< midT >::value ,
			  typename std::remove_pointer< midT >::type,
			  midT >::type */>
        ...
        public:
			...
			size_t size()const{ return m_raw_data.size(); }
                ...
                /// @param const std::string& [ I ], 当前状态ID
                /// @param  const stDataItem& [ I ], 当前已经处理的数据内容
                /// @return 如果函数执行后需要自动转换状态返回true，否则返回false
                using enterFun_t = std::function< bool ( const std::string& , const stDataItem& ) > ;

                /// @brief 离开状态转换函数。函数原型为 std::string cbFun( const std::string& id , const stDataItem& data );
                /// @param id[ I ], 当前状态ID
                /// @param data[ I ]
                using leaveFun_t = std::function< std::string ( const std::string& , const stDataItem& ) > ;
                /// @brief 发生转换的时候调用这个函数
                using raiseFun_t = std::function< void ( const std::string& , const std::string& , const stDataItem& ) >;
                ...
                struct state: public __base
                {
                        ...
                        state( const std::string& id , emType type = NORMAL ): m_type( type ), m_id( id ){}
                        ~state(){}
			...
			state& operator=( state&& b )
			{
                ...
                struct trans : public __base
                {
                        ...
                        trans( const std::string& f , const std::string& to ): m_from( f ), m_to( to ){}
                ...
                using transTbl_t = std::map< std::string , trans* > ;
                using statTbl_t     = std::map< std::string , state * >;

        private:
                std::string     	__m_start;         // 起始状态
                std::string     	__m_end;           // 结束状态
                std::string     	__m_curr;          // 当前状态
                std::string             __m_next;
		...
		*/
                fsm2( const std::string& start , enterFun_t entfun ):
                ...
                fsm2( const std::string& start , enterFun_t entfun , leaveFun_t leavefun ):
                	__m_start( start ) , __m_curr( start ), __m_is_running( false )
                {
                        if( start.empty() ) throw ERR_STAT_ID_EMPTY;
			...
			s->cb_enter = entfun;
                ...
                fsm2( const std::string& start , const std::string& end )
                ...
                virtual ~fsm2()
		{
			...
			for( auto it : __m_stat ){
                 ...
                 */
                emErrCode add( const std::string stat , enterFun_t ent )
                {
                        emErrCode ret = OK;
                 ...
                 */
                emErrCode add( const std::string stat , leaveFun_t leave )
                {
                        emErrCode ret = OK;
                        if( stat.empty() ) return ERR_STAT_ID_EMPTY;
			...
			return ret;
                 ...
                 * @param leave[ I ],
                 */
                emErrCode add( const std::string stat, enterFun_t ent , leaveFun_t leave )
                {
                        emErrCode ret = OK;
                        if( stat.empty() ) return ERR_STAT_ID_EMPTY;
                 ...
                 */
                emErrCode add( const std::string from , const std::string to , enterFun_t ent  )
                {
                        if( !__has_stat( from )){
                                __add_stat( from );
                        }
			...
			return OK;
                 ...
                 * @param to[ I ]
                 */
                emErrCode add( const std::string from , const std::string to )
                {
                        emErrCode ret = OK;
		...
		emErrCode add( const std::string from , const std::string to , raiseFun_t raise )
                {
                        emErrCode ret = OK;
                        if( from.empty() ) return ERR_STAT_ID_EMPTY;
                        if( to.empty() ) return ERR_STAT_ID_EMPTY;
		 ...
		 emErrCode add( const std::string from , const std::string to , leaveFun_t leave  )
                {
                        emErrCode ret = OK;
                        if( from.empty() ) return ERR_STAT_ID_EMPTY;
                        if( to.empty() ) return ERR_STAT_ID_EMPTY;
			...
			if( __has_trans( from , to ) == false ){
                ...
                emErrCode add( const std::string from , const std::string to , leaveFun_t leave  , raiseFun_t raise )
                {
                        emErrCode ret = OK;
                        if( from.empty() ) return ERR_STAT_ID_EMPTY;
                        if( to.empty() ) return ERR_STAT_ID_EMPTY;
			...
			if( __has_trans( from , to ) == false ){
                 ...
                 * @brief
                 */
                emErrCode add( const std::string from , const std::string to , enterFun_t ent , raiseFun_t raise )
                {
                        emErrCode ret = OK;
			...
			if( !__has_trans( from , to )){
                 ...
                 *
                 */
                emErrCode add( const std::string from , const std::string to , enterFun_t ent , leaveFun_t leave )
                {
                        emErrCode ret = OK;
                        if( from.empty() ) return ERR_STAT_ID_EMPTY;
                        if( to.empty() ) return ERR_STAT_ID_EMPTY;
			...
			if( !__has_stat( to ) ){
                 ...
                 * @brief
                 */
                emErrCode add( const std::string from , const std::string to,  enterFun_t ent , leaveFun_t leave , raiseFun_t raise )
                {
                        emErrCode ret = OK;
                        if( from.empty() ) return ERR_STAT_ID_EMPTY;
                        if( to.empty() ) return ERR_STAT_ID_EMPTY;
		 ...
		 *   可以用户旁路触发的情况
		 */
		emErrCode transform( const std::string& next )
		{
			emErrCode ret = OK;
        ...
        */
        static std::string errMsg( emErrCode e )
        {
                 ...
                 * @brief 指定离开的回调函数
                 */
                emErrCode setLeave( const std::string& id , leaveFun_t fun )
                {
                        emErrCode ret = OK;
		...
		*/
        ...
        private:
                 ...
                 */
                bool __has_stat( const std::string& id )
                {
                 ...
                 * @return 如果节点存在则返回true，否则返回false
                 */
                bool __has_trans( const std::string& from , const std::string& end )
                {
                        bool ret = false;
                        ret = ( __m_trans.find( from + end ) != __m_trans.end() );
                        return ret;
                }
                void __add_stat( const std::string& name )
                {
                	assert( !name.empty() );
                 ...
                 */
                void __add_stat( const std::string name , enterFun_t ent )
                {
                	assert( !name.empty() );
                        state * s = new state( name );
                        if( ent ){
		...
		/**
		*/
                void __set_leave( const std::string name, leaveFun_t leave )
                {
                 ...
                 * @brief 添加状态
                 */
                void  __add_stat( const std::string name , enterFun_t ent , leaveFun_t leave )
                {
                	assert( !name.empty() );
                        state * s( name );
                        if( ent ){
                 ...
                 */
                void  __add_trans( const std::string from , const std::string to , raiseFun_t raise )
                {
                        if( !__has_stat( from ) ){
                                __add_stat( from );
                        }
		...
		void __add_trans( const std::string from , const std::string to)
                {
			trans * __t = new trans( from , to );
...
#define onFsm2ENT( type , fun ) [&]( const std::string& s, const type::stDataItem& data )->bool{ return fun( s , data ); }
#define onFsm2RAISE( type , fun ) [&]( const std::string& fs , const std::string& ts , const type::stDataItem& data){ fun( fs , ts , data ); }
#define onFsm2LEAVE( type , fun ) [&]( const std::string& s, const type::stDataItem& data )->std::string{ return fun( s , data ); }
#else
