...
const size_t rcFtp::DEFAULT_BUF_SIZE    = 2048;    // 默认缓冲区大小
...
{
        rcFtp::errCode ret = rcFtp::OK;

        rcFtp::stTransmit * desc = (rcFtp::stTransmit *)userdata;
        if( desc ){
                rcFtp * obj = desc->p_ftp;
                if( obj == nullptr ) return nmemb;

                desc->p_buffer = ( char *)realloc( desc->p_buffer , nmemb );
                if( desc == nullptr ){ return rcFtp::ERR_DESC_NULL; }
                memcpy( desc->p_buffer , ptr , nmemb );
                desc->m_buf_size = nmemb;
                desc->m_data_size = nmemb;
		ret = (rcFtp::errCode)obj->on_recv( desc );

                if( ret == CURL_WRITEFUNC_PAUSE ){ // 执行暂停操作
                        obj->on_pause( triTrue );
                }else if( ret == rcFtp :: ERR_CANCEL_TRANSFER ){ // 执行取消操作
...
size_t ftp_write_cb_data(char *ptr, size_t /*size*/, size_t nmemb, void *userdata)
{
        ...
        if( desc ){
                ...
                if( ret == CURL_WRITEFUNC_PAUSE ){ // 执行暂停操作
                        obj->on_pause( triTrue );
                }else if( (rcFtp::errCode)ret == rcFtp :: ERR_CANCEL_TRANSFER ){ // 执行取消操作
                        obj->on_pause( triFalse );
                }
        }
        return nmemb;
...
size_t ftp_read_callback(char *buffer, size_t size, size_t nitems, void *userdata)
{
	...
	rcFtp::stTransmit * obj = reinterpret_cast< rcFtp::stTransmit *>( userdata );
        if( obj && obj->p_ftp ){
...
int on_upld_progress(void *clientp, curl_off_t /*dltotal*/, curl_off_t dlnow, curl_off_t /*ultotal*/, curl_off_t ulnow)
{
	...
	rcFtp :: stTransmit * ctl_info = ( rcFtp::stTransmit *)clientp;
...
rcFtp :: rcFtp():p_ctx( nullptr ),m_port( 0 ),m_file_size( 0 ), m_d_u( triReady ),m_is_running( false )
{
...
rcFtp :: rcFtp( const stParams& params ):p_ctx( nullptr ),m_port( 0 ), m_params( params ),m_file_size( 0 ), m_d_u( triReady ),m_is_running( false )
{
...
rcFtp :: rcFtp( const std::string& url_str ):p_ctx( nullptr ),m_port( 0 ),m_file_size( 0 ), m_d_u( triReady ),m_is_running( false )
{
	try{
		URL url( url_str );
		m_local_path = getcwd( nullptr , 0 );
...
void rcFtp :: __on_progress( size_t now )
{
	if( upld_cb ){
		upld_cb( rcFtp :: STATUS_TRANSING , now );
	}
}

void rcFtp :: __on_progress_dwld( size_t now )
{
//	if( dwld_cb ){
//		dwld_cb( rcFtp :: STATUS_TRANSING , now );
//	}
}

size_t rcFtp :: on_read_data( char * buff , size_t reqLen )
{
	size_t ret = 0;
...
int rcFtp :: on_pause( tribool_t status )
{
	int ret = 0;
...
rcFtp :: errCode
rcFtp :: real_download( const std::string& from ,  const std::string& to )
{
        errCode ret = OK;
        std::ofstream ofs;
	...
	transit.p_buffer = ( char *)malloc( m_params.buf_size );
...
rcFtp :: errCode
rcFtp :: __real_download( const std::string& from , char * data )
{
    errCode ret = OK;
	...
	stTransmit  transit;
	transit.p_ftp  = this;		  transit.m_type     = triTrue;
    transit.m_from = from;        transit.p_os       = nullptr;
	...
	rst = curl_easy_setopt( p_ctx1 , CURLOPT_USERNAME , m_usr.c_str() );
	if( rst != CURLE_OK ){
		ERROR_MSG( curl_easy_strerror( rst ));
		return ERR_USER_OR_PSWD;
	}
	rst = curl_easy_setopt( p_ctx1 , CURLOPT_PASSWORD , m_pswd.c_str() );
	if( rst != CURLE_OK ){
		ERROR_MSG( curl_easy_strerror( rst ));
		return ERR_USER_OR_PSWD;
	}
...
void rcFtp :: wait_finish( bool forceFinish )
{
	if( forceFinish == true ){
		m_is_running = false;
	}
...
bool rcFtp :: download( const std::string& from , const std::string& dest , bool autoRun )
{
        bool ret = false;
...
void rcFtp :: remody_url( const std::string& from , const std::string& dest )
{
	std::string strF;
	if( from.find( "ftp://") == std::string::npos ){
		if( from.at( 0 ) != '/' ){
...
void rcFtp :: download( const std::string& from ,
			const std::string& dest, std::function< void ( errCode ,  size_t  ) > cb ,
			bool autoRun )
{
        dwld_cb = cb;
        m_files.erase( m_files.begin() , m_files.end() );
...
void rcFtp :: download( const std::string& from , char * buffer ,std::function< void ( errCode , size_t ) > cb , bool autoRun )
{
    dwld_cb = cb;
    m_files.erase( m_files.begin() , m_files.end() );
    m_dests.erase( m_dests.begin() , m_dests.end() );
    ...
    try{
        ...
        if( autoRun ){
            ...
            std::thread thd([=]{
                errCode e = __real_download( m_files[ 0 ] , buffer );
                if( e != OK ){
                        do_process_error( e );
                }
...
bool rcFtp :: download( const ArrayString& from , const ArrayString& dest , bool autoRun )
{
        bool ret = false;
...
void rcFtp :: download( const ArrayString& from , const ArrayString& dest , std::function< void ( errCode , size_t ) > cb, bool autoRun )
{
...
void rcFtp :: __do_process_err( errCode e )
{

}
bool rcFtp :: upload( const std::string& file , const std::string& remote , bool autoRun )
{
	bool ret = false;
...
void rcFtp :: upload( const std::string& file , const std::string& remote , std::function< void ( errCode , size_t )> cb , bool autoRun )
{
	std::string __remote = remote;
...
bool rcFtp :: del( const std::string& rem )
{
	bool ret = false;
	struct curl_slist * hl = nullptr;
	std::string cmd("DELE ") , path , host;
	URL url( rem );
	path = url.Path();
	...
	CURL * p_ctx1 = curl_easy_init( );
	if( p_ctx1 == nullptr ){
		ERROR_MSG( "初始化CURL操作环境失败." );
		return ret;
	}
	std::stringstream ss;
	ss << url.Protocol() << "://" << url.HostName() << ":" << url.HostPort();
	host = ss.str();
	rst = curl_easy_setopt( p_ctx1 , CURLOPT_URL , host.c_str() );
	if( rst != CURLE_OK ){
		ERROR_MSG( curl_easy_strerror( rst ));
	}
	...
	curl_easy_setopt(p_ctx1, CURLOPT_POSTQUOTE, hl);
...
bool rcFtp :: upload( const ArrayString& from , bool autoRun )
{
	bool ret = false;
...
void rcFtp :: upload( const ArrayString& file , std::function< void ( errCode , size_t )> cb , bool autoRun )
{
}
...
void rcFtp :: run( std::function< void ( errCode , size_t , int , const std::string& ) > cb )
{
	p_ctx = curl_easy_init();
	if( p_ctx == nullptr ){
		throw ERR_CTX_NULL;
...
rcFtp :: errCode
rcFtp :: __access( const std::string& url )
{
	errCode ret = OK;
...
rcFtp :: errCode
rcFtp :: access( const std::string& url , int type )
{
	errCode ret = OK;
...
void rcFtp :: ls( std::function< void ( errCode , ArrayString& ) > cb )
{
...
rcFtp :: errCode
rcFtp :: cd( const std::string& path )
{
	errCode ret = OK;
...
int rcFtp :: do_process_save_file( stTransmit * desc )
{
        int ret = 0;

        if( desc != nullptr && desc->p_os != nullptr ){
                if( desc->p_os->write( desc->p_buffer , desc->m_data_size ).bad() == false ){
...
void rcFtp :: do_process_cmd( stTransmit * desc )
{

}

int rcFtp :: on_recv( stTransmit * desc )
{
        int ret = 0;
        if( desc == nullptr ) return 0;
...
int rcFtp :: on_recv_data( stTransmit * desc )
{
        int ret = 0;
        if( desc == nullptr ) return 0;
				// 执行传输操作
	if( desc->p_final_buffer != nullptr ){
		...
		ERROR_MSG( "null memory pointer." );
	}

        return ret;
}

size_t rcFtp ::  __size( const std::string& file )
{
	size_t ret = 0;
	if( p_ctx ){
...
void rcFtp :: onSize( const char * data , size_t size )
{
	if( data == nullptr ){
		m_file_size = 0;
		return;
...
void rcFtp :: do_process_error( errCode e )
{
        switch( e ){
        case OK:
	default:break;
...
void rcFtp::setUser( const std::string& acc , const std::string& pswd )
{
	m_usr = acc;
	m_pswd = pswd;
}

bool rcFtp::__connect()
{
	bool ret = false;
	if( p_ctx ){
...
void rcFtp :: __onConnect( const char * data , size_t len )
{
...
void rcFtp :: __onConnectErr()
{
	ERROR_MSG( " set ftp obj fail" );

}

void rcFtp ::  __onChkInst( const char * data , size_t len )
{
	if( data != nullptr )
...
rcFtp :: errCode
rcFtp :: __real_upload( const std::string& local ,const std::string& remote )
{
	errCode ret = OK;
...
CURLcode rcFtp :: __init_upload_curl( CURL * ctx , stTransmit& trans )
{
...
std::ifstream rcFtp :: __init_local_file( const std::string& file )
{
	std::ifstream is;
...
void rcFtp :: __init_upload_trans_info( const std::string& local , const std::string& remote , stTransmit& trans )
{
	trans.p_ftp = this;
	trans.m_from = local;
